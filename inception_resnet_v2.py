...
def inference_VGG(images, weight_decay=0.0, reuse=None):
    #----tf.int32 to tf.bool
    #new_phase_train = tf.cast(phase_train,tf.bool)
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
                        ...
                        normalizer_fn=slim.batch_norm,
                        normalizer_params=batch_norm_params):
        return VGG16(images, reuse=reuse)


def VGG16(inputs, reuse=None, scope='VGG16'):
    with tf.variable_scope(scope, 'VGG16', [inputs], reuse=reuse):
